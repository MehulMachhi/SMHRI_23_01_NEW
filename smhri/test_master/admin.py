...
class AudiometerThresholdDecimatsAdmin(admin.ModelAdmin):
    list_display = ('right_ac_500', 'right_bc_500', 'right_ac_1000')

    fieldsets = [
        ('AudiometerThresholdDecimatsAdmin', {
            'fields': ['right_ac_500', 'right_bc_500', 'right_ac_1000', 'right_bc_1000','right_ac_2000',
...
class BloodTestAdmin(admin.ModelAdmin):
    list_display = ('blood_cholestrol', 'blood_urea', 'fasting_blood_glucose'
                    )

    fieldsets = [
        ('BloodTestAdmin', {
            'fields': [ 'blood_cholestrol', 'creatinine', 'blood_urea', 'fasting_blood_glucose','random_blood_glucose',
...
class Urine_ExaminationAdmin(admin.ModelAdmin):
    list_display = ('volume', 'transparency', 'deposit'
                    )

    fieldsets = [
        ('Urine_ExaminationAdmin', {
            'fields': ['volume', 'transparency', 'deposit', 'colour','sp_gravity',
...
class ComplaintsAdmin(admin.ModelAdmin):
    list_display = ('present_complaints', 'occupational_complaints', 'family_health_history'
                    )

    fieldsets = [
        ('ComplaintsAdmin', {
            'fields': ['present_complaints', 'occupational_complaints', 'family_health_history', 'personal_health_history',
...
class HematologyAdmin(admin.ModelAdmin):
    list_display = ('blood_group', 'hemoglobin', 'polymorphs'
                    )

    fieldsets = [
        ('HematologyAdmin', {
            'fields': ['blood_group', 'hemoglobin', 'total_wbc_count', 'polymorphs', 'lymphocytes', 'eosinophils',
...
class LungFunctionTestAdmin(admin.ModelAdmin):
    list_display = ('fvc', 'peak_exp_flow', 'pefr_predicted'
                    )

    fieldsets = [
        ('LungFunctionTestAdmin', {
            'fields': ['fvc', 'fev1', 'fev1_fvc', 'peak_exp_flow', 'fef50', 'fvc_predicted', 'fev1_predicted', 'fev1_fvc_predicted',
...
class MicroscopicExaminationAdmin(admin.ModelAdmin):
    list_display = ('red_blood_cells', 'pus_cells', 'epithelial_cells'
                    )

    fieldsets = [
        ('MicroscopicExaminationAdmin', {
            'fields': ['red_blood_cells', 'pus_cells', 'epithelial_cells', 'urine_report', 'casts', 'crystais', 'material'],
        }),
...
class OtherTestsAdmin(admin.ModelAdmin):
    list_display = ('serum_cholinesterase', 'hs_crp', 'gppd'
                    )

    fieldsets = [
        ('OtherTestsAdmin', {
            'fields': ['serum_cholinesterase', 'hs_crp', 'gppd', 'hba1c', 'avg_blood_glucose_level', 'hbsag',
...
class PhysiologicalTestAdmin(admin.ModelAdmin):
    list_display = ('height', 'weight', 'blood_pressure'
                    )

    fieldsets = [
        ('PhysiologicalTestAdmin', {
            'fields': ['height', 'weight', 'blood_pressure', 'pulse', 'heart_sound', 'chest_on_expiration', 'chest_on_inspiration',
...
class SystematicExaminationAdmin(admin.ModelAdmin):
    list_display = ('skin', 'respiratory_system', 'cardiovascular_system'
                    )

    fieldsets = [
        ('SystematicExaminationAdmin', {
            'fields': ['skin', 'respiratory_system', 'cardiovascular_system', 'genito_urinary_system', 'skeletal_system',
...
class VisualTestAdmin(admin.ModelAdmin):
    list_display = ('nearvision_without_glass', 'distance_vision_without_glass', 'nearvision_with_glass'
                    )

    fieldsets = [
        ('VisualTestAdmin', {
            'fields': ['nearvision_without_glass', 'distance_vision_without_glass', 'nearvision_with_glass', 'distance_vision_with_glass',
...
class TestMasterAdmin( admin.ModelAdmin):
    list_display = ('test_name', 'test_desc', 'status'
                    )
