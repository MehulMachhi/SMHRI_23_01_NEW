...
def add_test_bloodtest(request):
    if request.method == "POST":
        ...
        companydata = BloodTest(blood_cholestrol=blood_cholestrol, creatinine=creatinine, blood_urea=blood_urea, fasting_blood_glucose=fasting_blood_glucose,random_blood_glucose=random_blood_glucose,
...
def add_urine_examination(request):
    if request.method == "POST":
        ...
        if str('companydata') == 'Yes':
            print('in if ')
        companydata = Urine_Examination(volume=volume, transparency=transparency, deposit=deposit, colour=colour,
...
def add_test_complaints(request):
    if request.method == "POST":
        ...
        if str('companydata') == 'Yes':
            print('in if ')
        companydata = Complaints(present_complaints=present_complaints, occupational_complaints=occupational_complaints, family_health_history=family_health_history, personal_health_history=personal_health_history,
...
def add_test_hematology(request):
    if request.method == "POST":
        ...
        if str('companydata') == 'Yes':
            print('in if ')
        companydata = Hematology(blood_group=blood_group, hemoglobin=hemoglobin, total_wbc_count=total_wbc_count, polymorphs=polymorphs, lymphocytes=lymphocytes, eosinophils=eosinophils, monocytes=monocytes,
...
def add_test_lungfunctiontest(request):
    if request.method == "POST":
        ...
        companydata = LungFunctionTest(fvc=fvc, fev1=fev1, fev1_fvc=fev1_fvc,peak_exp_flow=peak_exp_flow, fef50=fef50, fvc_predicted=fvc_predicted,fev1_predicted=fev1_predicted,
...
def add_test_microscopicexamination(request):
    if request.method == "POST":
        ...
        companydata = MicroscopicExamination(red_blood_cells=red_blood_cells, pus_cells=pus_cells, epithelial_cells=epithelial_cells, urine_report=urine_report, casts=casts,
...
def add_test_othertests(request):
    if request.method == "POST":
        ...
        companydata = OtherTests(serum_cholinesterase=serum_cholinesterase, hs_crp=hs_crp,gppd=gppd, hba1c=hba1c, avg_blood_glucose_level=avg_blood_glucose_level,hbsag=hbsag, hiv_result=hiv_result,
                 ...
                 stool_occult_blood=stool_occult_blood, stool_reducing_substances=stool_reducing_substances, stool_rbcs=stool_rbcs, stool_puscells=stool_puscells, stool_fat_globules=stool_fat_globules,
...
def add_test_physiologicaltest(request):
    if request.method == "POST":
        ...
        companydata = PhysiologicalTest(height=height, weight=weight, blood_pressure=blood_pressure, pulse=pulse, heart_sound=heart_sound, chest_on_expiration=chest_on_expiration,
...
def add_test_systematicexamination(request):
    if request.method == "POST":
        ...
        companydata = SystematicExamination(skin=skin, respiratory_system=respiratory_system, cardiovascular_system=cardiovascular_system, genito_urinary_system=genito_urinary_system,
...
def add_test_visualtest(request):
    if request.method == "POST":
        ...
        companydata = VisualTest(nearvision_without_glass=nearvision_without_glass, distance_vision_without_glass=distance_vision_without_glass,nearvision_with_glass=nearvision_with_glass,
